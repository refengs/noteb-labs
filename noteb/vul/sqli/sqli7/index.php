...
<div class="container">
    <section id="passReset" class=" center-block">
        <h2 class="passReset-title text-center">简单过滤的注入</h2>
		...
		if (empty($_GET['id'])) {
			...
			// if (preg_match('/(and|or|union|where|limit|group by|select|\'|hex|substr|\s)/i', $id)) {
			// 	echo "<script>alert('请不要尝试注入')</script>";
			// 	$id=1;
			// }else{
			// 	$id = $_GET['id'];
            ...
            $fw = new writeFW;
            $fw->fw($id,"sqli7.txt");
			$sql = "select * from sqli_7 where id ='$id' limit 0,1";
