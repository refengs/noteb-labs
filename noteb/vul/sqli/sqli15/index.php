...
<div class="container">
    <section id="passReset" class=" center-block">
        <h2 class="passReset-title text-center">POST注入6:有过滤的注入</h2>
        ...
        if (empty($_POST['username']) || empty($_POST['password'])) {
            ...
            if (preg_match('/(and|or|union|where|limit|group by|select|hex|substr|\s)/i', $_POST['username'])) {
                echo "<script>alert('请不要尝试注入')</script>";
                $username="";
            }else{
            ...
            $fw = new writeFW;
            $fw->fw($wt,"sqli15.txt");
            $sql = "select username,password from sqli_15 where username=('$username') and password=('$password') limit 0,1";
