...
<div class="container">
    <section id="passReset" class=" center-block">
        <h2 class="passReset-title text-center">多过滤一点的注入</h2>
		...
		if (empty($_GET['id'])) {
			...
			//从URL获取的ID赋值给$id
			if (preg_match('/(and|or|union|where|limit|group by|select|\'|hex|substr|\s)/i', $_GET['id'])) {
            ...
            $fw = new writeFW;
            $fw->fw($id,"sqli8.txt");
			$sql = "select * from sqli_8 where id ='$id' limit 0,1";
