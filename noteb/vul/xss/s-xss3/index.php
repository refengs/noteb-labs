...
include("../../sqli/sql-connect.php");
 ?>
<script>
confirm("这关老实点吧，弹窗了自己拿源代码的flag");
window.alert = function()  
{     
confirm("flag{get_sxss3-eeeee!!!}");
}
</script>
...
if (empty($_POST['text'])) {
    ...
    $content=$_POST['text'];
    $content2=str_ireplace("script","&nbsp;",$content);
    $content3=str_ireplace("on","&nbsp;",$content2);
    $content4=str_replace(" ","&nbsp;",$content3);
    $content5=str_replace("java","&nbsp;",$content4);
    $content6=str_replace("href","&nbsp;",$content5);
    $content7=str_replace("confirm","&nbsp;",$content6);
    $content8=str_replace("alert","&nbsp;",$content7);
    $content9=str_replace("prompt","&nbsp;",$content8);
    $content=str_replace("  ","&nbsp;",$content9);
//使用预编译，防止注入
    $sql = "INSERT INTO `s-xss3`(`text`) values (?) ";
...
<?php 
    $sql = "SELECT * FROM `s-xss3` order by id DESC ";
