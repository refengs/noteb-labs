...
<?php 
if (!empty($_GET['yzm'])&&($_GET['yzm'] == $_SESSION["findpwdyzm"])) {
	if ((!empty($_POST['username']))&&!empty($_POST['password'])) {
	    ...
	    echo $mysqli_stmt->error."<script>alert('重置密码失败！！！');";
	    }
	    $mysqli_stmt->close();
	    $mysqli->close();    
	 }	
...
if (!empty($_GET['username'])) {
 	...
 	echo $user;
				...
				<div>
				<div class="fomItem">
				<p><em>新密码：</em><input name="password" type="text" id="fuser_account" size="40" placeholder="请输入新密码" value="" /></p>
				</div>
				</div>
				</div>
				<div class="subCont">
            		<p><input class="btn btn-success btn-large" type="submit" name="button" id="fbutton" value="重置密码"></p>
				</div>
			</form>
		</div>
	</div>
