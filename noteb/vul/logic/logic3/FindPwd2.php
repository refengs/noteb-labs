...
<div id="wrapper">
    ...
    <div class="forgetCont forgerCt1">
    	<h3 class="titTop"></h3>
        <div class="forgetFom">
        	<form action="" method="GET" id="fgform">
       			<div class="fomItem">
...
if (!empty($user)&&empty($_GET['yzm'])) {
 	$yzm = rand(1000,9999);
 	if(!empty($_SESSION['test_yzm'])){
 		unset($_SESSION['test_yzm']);
 	}
 	$_SESSION['test_yzm'] = $yzm;
 }
				 ?>" /></p>
<?php 
if (!empty($_GET['yzm'])) {
	if ($_GET['yzm'] == $_SESSION['test_yzm']) {
		echo '<script type="text/javascript">window.location.href="./FindPwd3.php?username='.$user.'&yzm='.$_SESSION['test_yzm'].'&button=下一步";
    　　</script>';
	}else{
...
<!--end wrapper-->
<?php //echo "<script>console.log(".$yzm.")</script>"; ?>
