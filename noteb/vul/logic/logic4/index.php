  ...
  <div class="form-inner">
    ...
    <div class="input-wrapper">
      ...
      <div class="input-group"><span class="icon">
        ...
        <input type="password" id="login-password" data-lpignore="true" name="password">
      </div>
    </div>
    <div class="btn-group">
<!--       <button >登录</button> -->
...
if (!empty($_POST['username'])&&!empty($_POST['password'])) {
  ...
  $sql = "select * from `test_user` where username=(?) and password=(?) limit 0,1";
  ...
  //绑定结果集
  $mysqli_stmt->bind_result($id,$name,$pwd,$mony);
  //4、执行
  $b = $mysqli_stmt->execute();
  //获取结果
  $mysqli_stmt->fetch();
  if (!empty($name)&&!empty($pwd)) {
    if (!empty($_SESSION['test_user'])) {
      unset($_SESSION['test_user']);
    }
    if (!empty($_SESSION['test_mony'])) {
      unset($_SESSION['test_mony']);
    }
    $_SESSION['test_user'] = $name;
    $_SESSION['test_mony'] = $mony;
    echo '<script type="text/javascript">window.location.href="cat.php";
    　　</script>';
...
<p>Note-<a href="" target="_blank">B</a><p>
</div>
